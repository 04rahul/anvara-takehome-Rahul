...
router.get('/', async (req: Request, res: Response) => {
  try {
    const { type, available, publisherId, minPrice, maxPrice, sortBy, search, category } =
      req.query;
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const id = getParam(req.params.id);
      const { campaignId, creativeId, agreedPrice, pricingModel, startDate, endDate, message } =
        req.body;
      ...
      if (campaign.sponsorId !== sponsorId) {
        res
          .status(403)
          .json({ error: 'Forbidden: You can only request placements for your own campaigns' });
        return;
      }
      ...
      if (start < campaignStart) {
        res
          .status(400)
          .json({
            error: `Start date must be on or after campaign start date: ${campaignStart.toISOString().split('T')[0]}`,
          });
        return;
      }
      if (end > campaignEnd) {
        res
          .status(400)
          .json({
            error: `End date must be on or before campaign end date: ${campaignEnd.toISOString().split('T')[0]}`,
          });
        return;
      }
      ...
      if (existingPlacement) {
        const statusMessage =
          existingPlacement.status === 'PENDING'
      ...
      if (
        ...
        pricingModel !== null &&
        String(pricingModel).trim().length > 0
      ) {
        ...
        if (!Object.values(PricingModel).includes(candidate)) {
          res
            .status(400)
            .json({
              error: `Invalid pricingModel. Must be one of: ${Object.values(PricingModel).join(', ')}`,
            });
          return;
        }
      ...
      try {
        resolvedAgreedPrice = validateDecimal(agreedPrice, 'agreedPrice', {
          required: true,
          min: 0,
          max: 999999.99,
        });
      ...
      if (error && typeof error === 'object' && 'message' in error) {
        ...
        if (prismaError.code === 'P2002') {
          res
            .status(409)
            .json({ error: 'A placement request already exists for this combination' });
          return;
        }
    ...
    try {
      ...
      try {
        ...
        if (resolvedType === AdSlotType.PODCAST) {
          ...
          if (width !== undefined) {
            updateData.width = validateInteger(width, 'width', {
              required: false,
              min: 1,
              max: 10000,
              allowNull: true,
          ...
          if (height !== undefined) {
            updateData.height = validateInteger(height, 'height', {
              required: false,
              min: 1,
              max: 10000,
              allowNull: true,
        ...
        if (basePrice !== undefined) {
          updateData.basePrice = validateDecimal(basePrice, 'basePrice', {
            required: true,
            min: 0,
            max: 999999.99,
          });
